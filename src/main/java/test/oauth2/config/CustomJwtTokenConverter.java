...
public class CustomJwtTokenConverter extends JwtAccessTokenConverter {
    ...
    /* /oauth/token 요청에서 additional information 추가해 return */
    @Override
    ...
    protected String encode(OAuth2AccessToken accessToken, OAuth2Authentication authentication) {
        try {
            return AES256.encrypt(super.encode(accessToken, authentication), jwtProperties.getJWT_ENCRYPT_KEY());
        } catch (Exception e) {
            log.error("error: " , e);
            return null;
        }
    ...
    protected Map<String, Object> decode(String token) {
        try {
            if(token.contains(".")){
                ...
                String decoded = AES256.decrypt(token, jwtProperties.getJWT_ENCRYPT_KEY(), jwtProperties.getJWT_ENCRYPT_IV());
